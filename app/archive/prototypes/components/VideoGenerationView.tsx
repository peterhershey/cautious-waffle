...
export function VideoGenerationView({
  ...
  >(null);
  const downloadTimersRef = useRef<number[]>([]);
  ...
  const triggerDownload = useCallback(() => {
    ...
    downloadTimersRef.current.push(
      window.setTimeout(() => setDownloadStatus("downloaded"), 2800),
    );
    downloadTimersRef.current.push(
      window.setTimeout(() => setDownloadStatus(null), 6500),
    );
  }, [clearDownloadTimers]);
...
function InputPill({
  ...
  // Track whether the chip is still mounted (including during its exit
  // animation). Using `showChip` directly for `data-expanded` would flip the
  // grid template back to the single-row layout while AnimatePresence is
  // still playing the chip's exit, leaving the chip auto-placed onto an
  // implicit row and squeezing the placeholder text.
  const [chipPresent, setChipPresent] = useState(false);
  useEffect(() => {
    if (showChip) setChipPresent(true);
  }, [showChip]);
  ...
  return (
    ...
    >
      ...
      <motion.div layout className="gemini-input-row">
        ...
        <AnimatePresence
          initial={false}
          onExitComplete={() => setChipPresent(false)}
        >
          {showChip ? (
            <motion.div
              key="chip"
...
function DownloadToast({
  ...
  return (
    <motion.div
      className="gemini-download-toast"
      data-status={status}
...
const PLAN_THUMB_SRC = "/prototypes/video-generation/plan-thumb.png";
...
function VideoPlanCard({
  ...
  // Mock the planner generating a thumbnail on the fly: shimmer for a beat,
  // then fade the real image in.
  const [thumbReady, setThumbReady] = useState(false);
  useEffect(() => {
    const id = window.setTimeout(() => setThumbReady(true), 2600);
  ...
  return (
    ...
    >
      <div className="gemini-plan-header">
        <div
          className="gemini-plan-thumb"
          data-state={thumbReady ? "ready" : "loading"}
          aria-hidden
        >
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img
            className="gemini-plan-thumb-img"
            src={PLAN_THUMB_SRC}
            alt=""
            data-visible={thumbReady}
          />
        </div>
