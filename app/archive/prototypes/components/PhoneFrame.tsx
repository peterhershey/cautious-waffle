...
import type { ToastPayload } from "@/lib/prototypes";
...
export function PhoneFrame({
  ...
  useEffect(() => {
    console.log('[zoom-debug] PhoneFrame MOUNT');
    return () => console.log('[zoom-debug] PhoneFrame UNMOUNT');
  }, []);

  useEffect(() => {
    const setAt = Date.now();
    console.log('[zoom-debug] zoomExpanded changed to', zoomExpanded, '@', setAt);
    if (!zoomExpanded) return;
    collapseTimerRef.current = window.setTimeout(
      () => {
        const firedAt = Date.now();
        console.log('[zoom-debug] auto-collapse timer fired after', firedAt - setAt, 'ms');
        setZoomExpanded(false);
      },
      ZOOM_AUTOCOLLAPSE_MS,
    );
    return () => {
      console.log('[zoom-debug] cleanup, ref=', collapseTimerRef.current);
  ...
  return (
    <>
      {videoOn && backgroundVideo ? (
        <video
          className="proto-phone-backlight"
          src={backgroundVideo}
          autoPlay
          muted
          loop
        ...
        <div className="proto-phone-backlight" data-mode="gel" aria-hidden />
      )}
    <div
      ...
      <div className="proto-phone-screen">
                  ...
                  {ZOOM_LEVELS.map((z) => {
                    ...
                    return (
                      <motion.button
                        ...
                        onClick={() => {
                          console.log('[zoom-debug] onClick fired, z=', z, 'zoomExpanded=', zoomExpanded, 'zoom=', zoom);
                          if (!zoomExpanded) {
                          ...
                          console.log('[zoom-debug] scheduling collapse with delay=', delay);
                          if (z !== zoom) setZoom(z);
                          collapseTimerRef.current = window.setTimeout(
                            () => {
                              console.log('[zoom-debug] click-handler timer fired (delay was', delay, ')');
                              setZoomExpanded(false);
                              collapseTimerRef.current = null;
                            },
                            delay,
        ...
        <AnimatePresence>
          {toast ? (
            ...
            >
              <span className="proto-toast-icon" aria-hidden>
                <IconWarning />
              </span>
              <span className="proto-toast-body">
                <span className="proto-toast-title">{toast.title}</span>
                {toast.body ? (
                  <span className="proto-toast-detail">{toast.body}</span>
                ) : null}
              </span>
            </motion.div>
          ) : null}
